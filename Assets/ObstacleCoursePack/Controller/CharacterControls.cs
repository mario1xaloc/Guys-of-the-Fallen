...
public class CharacterControls : NetworkBehaviour
{
    ...
    public CameraManager camPrefab;
    ...
    void Start()
    {
        ...
        if (isLocalPlayer)
        {
            //Search for old cameras
            float maxDepth = 0;
            Camera[] oldCameras = FindObjectsOfType<Camera>();
            for (int i = 0; i < oldCameras.Length; i++)
            {
                //oldCameras[i].gameObject.SetActive(false);
                if (oldCameras[i].depth > maxDepth)
                    maxDepth = oldCameras[i].depth;
            }

            cam = Instantiate(camPrefab);
            cam.target = transform;
            cam.SetDepth(maxDepth + 1);
        }
        else 
        {
            Destroy(this);
